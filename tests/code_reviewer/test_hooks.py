"""
Tests for sample_gh_copilot_sdk.code_reviewer.hooks.
...
from sample_gh_copilot_sdk.code_reviewer.hooks import (
    build_hooks,
    on_post_tool_use,
    on_pre_tool_use,
    on_session_start,
...
class TestOnPreToolUse:
    ...
    def test_includes_additional_context(self) -> None:
        ...
        :since: 0.1.0
        """
        result = asyncio.run(on_pre_tool_use({"toolName": "grep"}, {}))
...
class TestOnSessionStart:
    ...
    def test_returns_additional_context(self) -> None:
        ...
        :since: 0.1.0
        """
        result = asyncio.run(on_session_start({"source": "test"}, {}))
        assert "additionalContext" in result

    def test_handles_missing_source(self) -> None:
        """Missing source in input_data must not raise an exception.
    ...
    def test_context_mentions_inline_agent(self) -> None:
        """The context should indicate the inline agent was initialised.

        :author: Ron Webb
        :since: 0.1.0
        """
        result = asyncio.run(on_session_start({}, {}))
        assert "inline" in result["additionalContext"].lower()


class TestBuildHooks:
    """Tests for the build_hooks factory.

    :author: Ron Webb
    :since: 0.1.0
    """

    def test_returns_dict_with_three_hooks(self) -> None:
        """build_hooks() must return a dict with exactly three entries.
    ...
    def test_all_hook_keys_present(self) -> None:
        """All three required hook keys must be present in the returned dict.
    ...
    def test_all_hooks_are_callable(self) -> None:
        """All hook values must be callable.
